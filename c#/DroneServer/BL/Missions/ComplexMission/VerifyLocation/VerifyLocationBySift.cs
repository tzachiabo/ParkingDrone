...
{
    class VerifyLocationBySift : ComplexMission
    {
        Point start_location;
        Response res;

        public VerifyLocationBySift(ComplexMission ParentMission = null) : base(ParentMission)
        {
        ...
        {
            Logger.getInstance().info("Start sift verify location");
        ...
        public override void done(Response response)
        {
            ...
            Logger.getInstance().info("After sift verify location drone location is margin-left: " + p.lng + " margin-top " + p.lat);
            base.done(response);
        }
    }
}
